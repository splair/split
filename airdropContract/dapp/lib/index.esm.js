import { TransactionInstruction, PublicKey, Account, SystemProgram, Transaction, sendAndConfirmTransaction } from '@solana/web3.js';
import { u64, Token, AccountLayout } from '@solana/spl-token';
...
class Airdrop {
  ...
  static createAirdropInstruction(programID, publisherKey, bankKey, mintKey, airdropKey, authKey, amount, count, nonce, names) {
    const dataLayout = struct([u8("i"), blob(8, "amount"), blob(8, "count"), u8("nonce")]);
  ...
  static async airdrop(connection, programID, publisherAccount, mintKey, amount, names) {
    ...
    const balanceNeeded = await Token.getMinBalanceRentForExemptAccount(connection); // step3: create createAccount transaction instrument
    ...
    const initAccountTrx = Token.createInitAccountInstruction(new PublicKey(TOKEN_PROGRAM), mintKey, splAccount.publicKey, authKey);
...
export { Airdrop, airdropHeadSpace, publicKey };
